...
const newPerson = new Person('Neo',20);
...
class Car {
  ...
  drive(distance){
    ...
    if (drivableMiles > distance){
      ...
      return `I drove ${distance} miles and have ${this.tank} gallons left in my tank`;
    } else {
      this.tank = 0;
      this.odometer += drivableMiles;
      return `We ran out of gas after ${drivableMiles} miles`;
    }
  }
}
...
class Instructor extends Lambdasian{
  ...
  scoring(studentObj){
    studentObj.grade += ((Math.floor(Math.random()*11))-5)
  }
}
...
class Student extends Lambdasian{
  ...
  graduate(){
    if (this.grade >= 70){
      return `Congrats, you've graduated!`;
    } else {
      return `Sorry, you're flexxing bud.`
    }
  }
}
